...
/*CREAR DATABASE LINK*/
CREATE DATABASE LINK store1 CONNECT TO SYSTEM IDENTIFIED BY "123456" USING
...
/*TABLAS REPLICAS*/
DROP TABLE CUSTOMERSREPLICAA;
DROP TABLE ORDER_ITEMSREPLICAA;
DROP TABLE ORDERSREPLICAA;
DROP TABLE PRODUCT_INFORMATIONREPLICAA;

CREATE TABLE CUSTOMERSREPLICAA (
     CUSTOMER_ID NUMBER(6,0), 
     CUST_FIRST_NAME VARCHAR2(20), 
     CUST_LAST_NAME VARCHAR2(20), 
     CREDIT_LIMIT NUMBER(9,2), 
...
CREATE TABLE ORDER_ITEMSREPLICAA 
    (ORDER_ID NUMBER(12), 
    LINE_ITEM_ID NUMBER(3), 
    PRODUCT_ID NUMBER(6), 
    UNIT_PRICE NUMBER(8), 
...
CREATE TABLE PRODUCT_INFORMATIONREPLICAA 
    (PRODUCT_ID NUMBER(6), 
...
/*SINONIMOS*/  
CREATE SYNONYM CUSTOMERSSTOREA
FOR CUSTOMERS@STORE1;
SELECT *
FROM CUSTOMERSSTOREA;

CREATE SYNONYM ORDER_ITEMSSTOREA
FOR ORDER_ITEMS@STORE1;
SELECT *
FROM ORDER_ITEMSSTOREA;

CREATE SYNONYM ORDERSSTOREA
FOR ORDERS@STORE1;
SELECT *
FROM ORDERSSTOREA;

CREATE SYNONYM PRODUCT_INFORMATIONSTOREA
FOR PRODUCT_INFORMATION@STORE1;
SELECT *
FROM PRODUCT_INFORMATIONSTOREA;

CREATE SYNONYM replicaCustomerEnA
FOR CUSTOMERSREPLICAB@STORE1;
SELECT *
FROM replicaCustomerEnA;

CREATE SYNONYM replicaOrder_ItemEnA
FOR ORDER_ITEMSREPLICB@STORE1;
SELECT *
FROM replicaOrder_ItemEnA;

CREATE SYNONYM replicaOrdersEnA
FOR ORDERSREPLICAB@STORE1;
SELECT *
FROM replicaOrdersEnA;
...
AS
BEGIN
    IF p_REGION = 'C' OR p_REGION = 'D' THEN
    ...
    ELSIF p_REGION = 'A' OR p_REGION = 'B' THEN
        INSERT INTO CUSTOMERSSTOREA (CUSTOMER_ID, CUST_FIRST_NAME, CUST_LAST_NAME, CREDIT_LIMIT, CUST_EMAIL, INCOME_LEVEL, REGION)
        VALUES (p_CUSTOMER_ID, p_CUST_FIRST_NAME, p_CUST_LAST_NAME, p_CREDIT_LIMIT, p_CUST_EMAIL, p_INCOME_LEVEL, p_REGION);
    ELSE
...
/
BEGIN
    insertCustomer('983','Aldo','Martinez','2000','Aldo.Martinez@SCOTER.COM','E: 90,000 - 109,999','A');
END;
/
...
/
BEGIN
    actualizaCustomer('982','Brandon','Ceballos','2500','Brandon.Ceballos@SCOTER.COM','E: 90,000 - 109,999','C');
END;
/
SELECT *
FROM CUSTOMERS;
...
BEGIN
    ...
    IF SQL%ROWCOUNT = 0 THEN
        DELETE FROM CUSTOMERSSTOREA WHERE CUSTOMER_ID = p_CUSTOMER_ID;
    END IF;
...
/
DECLARE
    v_customer_id_to_delete NUMBER := 985;
BEGIN
...
BEGIN
    BEGIN
        DBMS_MVIEW.REFRESH('todosLosCustomers', 'C');
        SELECT REGION INTO v_REGION FROM todosLosCustomers WHERE CUSTOMER_ID = p_CUSTOMER_ID;
    EXCEPTION
        WHEN NO_DATA_FOUND THEN
            RAISE_APPLICATION_ERROR(-20001, 'Cliente no encontrado con el ID proporcionado.');
    END;
    IF v_REGION = 'A' OR v_REGION = 'B' THEN
        INSERT INTO ORDERSSTOREA (ORDER_ID, ORDER_DATE, ORDER_MODE, CUSTOMER_ID, ORDER_STATUS, ORDER_TOTAL, SALES_REP_ID, PROMOTION_ID)
...
BEGIN
    ...
    IF v_REGION IS NOT NULL THEN
        IF v_REGION = 'A' OR v_REGION = 'B' THEN
            DELETE FROM ORDERSSTOREA WHERE ORDER_ID = p_ORDER_ID;
        ELSIF v_REGION = 'C' OR v_REGION = 'D' THEN
...
BEGIN
    ...
    IF v_REGION = 'A' OR v_REGION = 'B' THEN
        INSERT INTO ORDER_ITEMSSTOREA (ORDER_ID, LINE_ITEM_ID, PRODUCT_ID, UNIT_PRICE, QUANTITY)
...
BEGIN
    insertarOrderItem(2385, 5, 2316, 21, 8);
END;
/
...
/
BEGIN
    actualizarOrderItem(2365, 1, 2289, 50, 90);
END;
/
...
BEGIN
    IF INSERTING OR UPDATING THEN
        INSERT INTO replicaCustomerEnA (CUSTOMER_ID, CUST_FIRST_NAME, CUST_LAST_NAME, CREDIT_LIMIT, CUST_EMAIL, INCOME_LEVEL, REGION)
    ...
    ELSE
        DELETE FROM replicaCustomerEnA
...
BEGIN
    IF INSERTING OR UPDATING THEN
        INSERT INTO replicaOrder_ItemEnA (ORDER_ID, LINE_ITEM_ID, PRODUCT_ID, UNIT_PRICE, QUANTITY)
        VALUES (:NEW.ORDER_ID, :NEW.LINE_ITEM_ID, :NEW.PRODUCT_ID, :NEW.UNIT_PRICE, :NEW.QUANTITY);
    ELSE
        DELETE FROM replicaOrder_ItemEnA
...
BEGIN
    IF INSERTING OR UPDATING THEN
        INSERT INTO replicaOrdersEnA (ORDER_ID, ORDER_DATE, ORDER_MODE, CUSTOMER_ID, ORDER_STATUS, ORDER_TOTAL, SALES_REP_ID, PROMOTION_ID)
...
/*VISTA MATERIALIZADA*/
CREATE MATERIALIZED VIEW todosLosCustomers
...
DROP MATERIALIZED VIEW todosOrders;
