...
/*CREAR DATABASE LINK*/
CREATE DATABASE LINK store2 CONNECT TO SYSTEM IDENTIFIED BY "123456" USING
...
/*TABLAS REPLICAS*/
DROP TABLE CUSTOMERSREPLICAB;
DROP TABLE ORDER_ITEMSREPLICAB;
DROP TABLE ORDERSREPLICABB;

CREATE TABLE CUSTOMERSREPLICAB(
     CUSTOMER_ID NUMBER(6,0), 
     CUST_FIRST_NAME VARCHAR2(20), 
     CUST_LAST_NAME VARCHAR2(20), 
     CREDIT_LIMIT NUMBER(9,2), 
...
CREATE TABLE ORDER_ITEMSREPLICB  
    (ORDER_ID NUMBER(12), 
    LINE_ITEM_ID NUMBER(3), 
    PRODUCT_ID NUMBER(6), 
    UNIT_PRICE NUMBER(8), 
...
/*SINONIMO0S*/
CREATE SYNONYM CUSTOMERSSTOREB
FOR CUSTOMERS@STORE2;
SELECT *
FROM CUSTOMERSSTOREB;

CREATE SYNONYM ORDER_ITEMSSTOREB
FOR ORDER_ITEMS@STORE2;
SELECT *
FROM ORDER_ITEMSSTOREB;

CREATE SYNONYM ORDERSSTOREB
FOR ORDERS@STORE2;
SELECT *
FROM ORDERSSTOREB;

CREATE SYNONYM PRODUCT_INFORMATIONSTOREB
FOR PRODUCT_INFORMATION@STORE2;
SELECT *
FROM PRODUCT_INFORMATIONSTOREB;

CREATE SYNONYM replicaCustomerEnB
FOR CUSTOMERSREPLICAA@STORE2;
SELECT *
FROM replicaCustomerEnB;

CREATE SYNONYM replicaOrder_ItemEnB
FOR ORDER_ITEMSREPLICAA@STORE2;
SELECT *
FROM replicaOrder_ItemEnB;

CREATE SYNONYM replicaOrdersEnB
FOR ORDERSREPLICAA@STORE2;
SELECT *
FROM replicaOrdersEnB;

CREATE SYNONYM replicaProductInformationEnB
FOR PRODUCT_INFORMATIONREPLICAA@STORE2;
SELECT *
FROM replicaProductInformationEnB;

/*VISTAS*/
CREATE OR REPLACE VIEW VistaCustomersGlobal AS
SELECT *
FROM CUSTOMERS
UNION ALL
SELECT *
...
AS
BEGIN
    IF p_REGION = 'A' OR p_REGION = 'B' THEN
    ...
    ELSIF p_REGION = 'C' OR p_REGION = 'D' THEN
        INSERT INTO CUSTOMERSSTOREB (CUSTOMER_ID, CUST_FIRST_NAME, CUST_LAST_NAME, CREDIT_LIMIT, CUST_EMAIL, INCOME_LEVEL, REGION)
        VALUES (p_CUSTOMER_ID, p_CUST_FIRST_NAME, p_CUST_LAST_NAME, p_CREDIT_LIMIT, p_CUST_EMAIL, p_INCOME_LEVEL, p_REGION);
    ELSE
        DBMS_OUTPUT.PUT_LINE('Region no valida');
    END IF;
END insertCustomer;
/
/*A PARTIR DEL 982*/
...
BEGIN
    ...
    IF SQL%ROWCOUNT = 0 THEN
        DBMS_OUTPUT.PUT_LINE('No hay un cliente con ese ID.');
    END IF;
EXCEPTION
    WHEN OTHERS THEN
...
/*INSERTAR*/
CREATE OR REPLACE PROCEDURE insertarOrder(
...
BEGIN
    BEGIN
        DBMS_MVIEW.REFRESH('todosCustomers', 'C');
        SELECT REGION INTO v_REGION FROM todosCustomers WHERE CUSTOMER_ID = p_CUSTOMER_ID;
    EXCEPTION
        WHEN NO_DATA_FOUND THEN
            RAISE_APPLICATION_ERROR(-20001, 'Cliente no encontrado con el ID proporcionado.');
    END;
    ...
    ELSIF v_REGION = 'C' OR v_REGION = 'D' THEN
        INSERT INTO ORDERSSTOREB (ORDER_ID, ORDER_DATE, ORDER_MODE, CUSTOMER_ID, ORDER_STATUS, ORDER_TOTAL, SALES_REP_ID, PROMOTION_ID)
        VALUES (p_ORDER_ID, p_ORDER_DATE, p_ORDER_MODE, p_CUSTOMER_ID, p_ORDER_STATUS, p_ORDER_TOTAL, p_SALES_REP_ID, p_PROMOTION_ID);
    ELSE
...
BEGIN
    ...
    IF SQL%ROWCOUNT = 0 THEN
        DBMS_OUTPUT.PUT_LINE('No hay una orden con ese ID.');
    END IF;
EXCEPTION
    WHEN OTHERS THEN
        DBMS_OUTPUT.PUT_LINE('Error: ' || SQLERRM);
...
BEGIN
    ...
    IF v_REGION IS NOT NULL THEN
        ...
        ELSIF v_REGION = 'C' OR v_REGION = 'D' THEN
            DELETE FROM ORDERSSTOREB WHERE ORDER_ID = p_ORDER_ID;
        END IF;

        IF SQL%ROWCOUNT = 0 THEN
            DBMS_OUTPUT.PUT_LINE('No hay una orden con ese ID.');
        ELSE
            DBMS_OUTPUT.PUT_LINE('Orden eliminada correctamente.');
        END IF;
    ELSE
        DBMS_OUTPUT.PUT_LINE('No se encontró un cliente asociado con la orden.');
    END IF;
EXCEPTION
    WHEN OTHERS THEN
        DBMS_OUTPUT.PUT_LINE('Error: ' || SQLERRM);
...
/*INSERTAR*/
CREATE OR REPLACE PROCEDURE insertarOrderItem(
    p_ORDER_ID IN NUMBER,
...
BEGIN
    ...
    EXCEPTION
        WHEN NO_DATA_FOUND THEN
            DBMS_OUTPUT.PUT_LINE('No se encontró una orden con el ID proporcionado.');
            RETURN;
        WHEN TOO_MANY_ROWS THEN
            DBMS_OUTPUT.PUT_LINE('Se encontró más de una orden con el mismo ID. Verifique los datos.');
            RETURN;
    END;
    ...
    EXCEPTION
        WHEN NO_DATA_FOUND THEN
            DBMS_OUTPUT.PUT_LINE('No se encontró un cliente con el ID proporcionado.');
            RETURN;
        WHEN TOO_MANY_ROWS THEN
            DBMS_OUTPUT.PUT_LINE('Se encontró más de un cliente con el mismo ID. Verifique los datos.');
            RETURN;
    END;
    ...
    ELSIF v_REGION = 'C' OR v_REGION = 'D' THEN
        INSERT INTO ORDER_ITEMSSTOREB (ORDER_ID, LINE_ITEM_ID, PRODUCT_ID, UNIT_PRICE, QUANTITY)
        VALUES (p_ORDER_ID, p_LINE_ITEM_ID, p_PRODUCT_ID, p_UNIT_PRICE, p_QUANTITY);
    ELSE
        DBMS_OUTPUT.PUT_LINE('Región no válida para el cliente.');
    END IF;
...
BEGIN
    ...
    IF SQL%ROWCOUNT = 0 THEN
        DBMS_OUTPUT.PUT_LINE('No hay un item de orden con ese ID.');
    END IF;
EXCEPTION
    WHEN OTHERS THEN
        DBMS_OUTPUT.PUT_LINE('Error: ' || SQLERRM);
...
BEGIN
    ...
    IF SQL%ROWCOUNT = 0 THEN
        DBMS_OUTPUT.PUT_LINE('No hay un item de orden con ese ID.');
    ELSE
        DBMS_OUTPUT.PUT_LINE('Item de orden eliminado correctamente.');
    END IF;
EXCEPTION
    WHEN OTHERS THEN
        DBMS_OUTPUT.PUT_LINE('Error: ' || SQLERRM);
...
/*PRODUCT_INRORMATION*/
/* INSERTAR */
CREATE OR REPLACE PROCEDURE insertarProductItem(
    p_PRODUCT_ID IN NUMBER,
...
EXCEPTION
    WHEN OTHERS THEN
        DBMS_OUTPUT.PUT_LINE('Error: ' || SQLERRM);
END insertarProductItem;
/

/* ACTUALIZAR */
CREATE OR REPLACE PROCEDURE actualizarProductItem(
    p_PRODUCT_ID IN NUMBER,
    p_PRODUCT_NAME IN VARCHAR2,
    p_PRODUCT_DESCRIPTION IN VARCHAR2,
    p_CATEGORY_ID IN NUMBER,
...
BEGIN
    ...
    IF SQL%ROWCOUNT = 0 THEN
        DBMS_OUTPUT.PUT_LINE('No hay un producto con ese ID.');
    END IF;
EXCEPTION
    WHEN OTHERS THEN
        DBMS_OUTPUT.PUT_LINE('Error: ' || SQLERRM);
END actualizarProductItem;
/

/* ELIMINAR */
CREATE OR REPLACE PROCEDURE eliminarProductItem(
    p_PRODUCT_ID IN NUMBER
)
AS
BEGIN
...
/*TRIGGERS*/
/*REPLICA DE CUSTOMERS*/
CREATE OR REPLACE TRIGGER customerReplica
AFTER INSERT OR UPDATE OR DELETE
ON CUSTOMERS
FOR EACH ROW
BEGIN
    IF INSERTING OR UPDATING THEN
        INSERT INTO replicaCustomerEnB (CUSTOMER_ID, CUST_FIRST_NAME, CUST_LAST_NAME, CREDIT_LIMIT, CUST_EMAIL, INCOME_LEVEL, REGION)
    ...
    ELSE
        DELETE FROM replicaCustomerEnB
...
/*REPLICA DE ORDER ITEMS*/
CREATE OR REPLACE TRIGGER order_ItemReplica
AFTER INSERT OR UPDATE OR DELETE
ON ORDER_ITEMS
FOR EACH ROW
BEGIN
    IF INSERTING OR UPDATING THEN
        INSERT INTO replicaOrder_ItemEnB (ORDER_ID, LINE_ITEM_ID, PRODUCT_ID, UNIT_PRICE, QUANTITY)
        VALUES (:NEW.ORDER_ID, :NEW.LINE_ITEM_ID, :NEW.PRODUCT_ID, :NEW.UNIT_PRICE, :NEW.QUANTITY);
    ELSE
        DELETE FROM replicaOrder_ItemEnB
...
/*REPLICA DE ORDERS*/
CREATE OR REPLACE TRIGGER ordersReplica
AFTER INSERT OR UPDATE OR DELETE
ON ORDERS
FOR EACH ROW
BEGIN
    IF INSERTING OR UPDATING THEN
        INSERT INTO replicaOrdersEnB (ORDER_ID, ORDER_DATE, ORDER_MODE, CUSTOMER_ID, ORDER_STATUS, ORDER_TOTAL, SALES_REP_ID, PROMOTION_ID)
...
/*REPLICA DE PRODUCTS INFORMATION*/
CREATE OR REPLACE TRIGGER product_informationReplica
AFTER INSERT OR UPDATE OR DELETE
ON PRODUCT_INFORMATION
FOR EACH ROW
BEGIN
    IF INSERTING OR UPDATING THEN
        INSERT INTO replicaProductInformationEnB (PRODUCT_ID, PRODUCT_NAME, PRODUCT_DESCRIPTION, CATEGORY_ID, WEIGHT_CLASS, WARRANTY_PERIOD, SUPPLIER_ID, PRODUCT_STATUS, LIST_PRICE, MIN_PRICE, CATALOG_URL)
    ...
    ELSE
        DELETE FROM replicaProductInformationEnB
...
CREATE MATERIALIZED VIEW todosOrdersItems
REFRESH COMPLETE ON DEMAND
AS
SELECT *
FROM ORDER_ITEMS
...
FROM (SELECT ORDER_ID, CUSTOMER_ID FROM ORDERS
      UNION ALL
      SELECT ORDER_ID, CUSTOMER_ID FROM ORDERSSTOREB) o
...
Insert into PRODUCT_INFORMATION (PRODUCT_ID,PRODUCT_NAME,PRODUCT_DESCRIPTION,CATEGORY_ID,WEIGHT_CLASS,WARRANTY_PERIOD,SUPPLIER_ID,PRODUCT_STATUS,LIST_PRICE,MIN_PRICE,CATALOG_URL) values (1779,'C for SPNIX3.3 - Doc','C programming language documentation, SPNIX V3.3',25,2,'10-0',103092,'orderable',128,112,'http://www.supp-103092.com/cat/sw/p1779.html');
